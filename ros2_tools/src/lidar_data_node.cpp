...
public:
  ...
  LidarDataNode() : Node("lidar_data_node") {
    ...
    // LidarPose 发布
    lidar_pub =
        this->create_publisher<ros2_tools::msg::LidarPose>("lidar_data", 10);
    ...
    RCLCPP_INFO(this->get_logger(),
                "PX4 Local Position subscription successful.");
  }
  ...
  // PX4数据回调
  void
  localPositionCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    if (using_gazebo)
  ...
  void msgDispose(const geometry_msgs::msg::Pose &pose) {
    ...
    if (++counter >= 50) {
      RCLCPP_INFO(
          this->get_logger(),
          "Position=(%.2f, %.2f, %.2f), Orientation=(%.2f, %.2f, %.2f) rad", x,
          y, z, roll, pitch, yaw);
      counter = 0;
    }
  }
